...
function createMailRow(email, options = {}) {
    ...
    return `
        <article class="mail-row ${important ? "important" : ""}" data-email="${encoded}">
            ...
            ${attachmentCount
            ? `<div class="mail-row-extra">Adjuntos: ${attachmentCount}</div>`
            : ""
        }
        </article>
    `;
    ...
    listFields.forEach(([key, label]) => {
        ...
        if (Array.isArray(value) && value.length) {
            rows.push(`
                <div class="data-row">
                    <span class="data-label">${label}</span>
                    <span class="data-value">${escapeHtml(value.join(", "))}</span>
                </div>
            `);
        }
    });
...
function abrirDetalle(email) {
    ...
    if (detectedHasContent) {
        detailDetectedBlock.classList.remove("hidden");
        detailDetectedData.innerHTML = renderDetectedData(email.extracted || {});
    } else {
        detailDetectedBlock.classList.add("hidden");
        detailDetectedData.innerHTML = "";
    }

    if (email.missing && email.missing.length) {
        detailMissingBlock.classList.remove("hidden");
        detailMissingData.innerHTML = renderMissingData(email.missing || []);
    } else {
        detailMissingBlock.classList.add("hidden");
        detailMissingData.innerHTML = "";
    }

    if (email.extraction_notes && email.extraction_notes.trim()) {
        detailExtractionBlock.classList.remove("hidden");
        detailExtractionNotes.textContent = email.extraction_notes;
    } else {
        detailExtractionBlock.classList.add("hidden");
        detailExtractionNotes.textContent = "";
    }
