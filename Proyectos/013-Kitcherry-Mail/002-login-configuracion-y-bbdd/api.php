...
// ==========================================================
// KITCHERRY MAIL
// API interna para trabajar contra SQLite.
// ==========================================================
...
try {
    ...
    switch ($accion) {
        case 'sync':
            responderOk($pdo, $accountId, 'La sincronización real se añadirá con IMAP.');
            break;
...
function actualizarLectura(PDO $pdo, int $accountId, array $entrada): void
{
    ...
    if (!$silencioso) {
        registrarHistorial($pdo, $id, $leido ? 'Marcado como leído.' : 'Marcado como no leído.');
    }
...
function moverCarpeta(PDO $pdo, int $accountId, array $entrada): void
{
    ...
    if ($carpeta === 'archived') {
        registrarHistorial($pdo, $id, 'Correo archivado.');
        responderOk($pdo, $accountId, 'Correo archivado');
    }

    if ($carpeta === 'trash') {
        registrarHistorial($pdo, $id, 'Correo enviado a papelera.');
        responderOk($pdo, $accountId, 'Correo enviado a papelera');
    }

    registrarHistorial($pdo, $id, 'Correo movido de carpeta.');
...
function actualizarEstado(PDO $pdo, int $accountId, array $entrada): void
{
    ...
    registrarHistorial($pdo, $id, 'Estado cambiado a ' . textoEstadoServidor($estado) . '.');
...
function agregarNota(PDO $pdo, int $accountId, array $entrada): void
{
    ...
    registrarHistorial($pdo, $id, 'Nota interna añadida.');
...
function crearCorreoLocalDesdeEditor(PDO $pdo, int $accountId, array $entrada, string $carpeta, string $estado, string $historial): int
{
    ...
    registrarHistorial($pdo, $id, $historial);
 ...
 * Registra historial.
 */
function registrarHistorial(PDO $pdo, int $emailId, string $accion): void
{
    $stmt = $pdo->prepare("
