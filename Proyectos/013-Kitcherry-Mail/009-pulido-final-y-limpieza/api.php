...
// Incluye mantenimiento final de base local.
// ==========================================================
...
/**
 * Devuelve estadísticas para mantenimiento.
 */
...
/**
 * Calcula estadísticas útiles para mantenimiento.
 */
...
function limpiarCorreosSoloLocales(PDO $pdo, int $accountId): void
{
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Correos locales eliminados: ' . $total,
        'stats' => calcularEstadisticasMantenimiento($pdo, $accountId),
...
function vaciarPapeleraLocal(PDO $pdo, int $accountId): void
{
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Papelera local vaciada: ' . $total,
        'stats' => calcularEstadisticasMantenimiento($pdo, $accountId),
        'correos' => obtenerCorreosParaVista($pdo, $accountId)
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
...
/**
 * Borra todos los mensajes locales y vuelve a sincronizar desde servidor.
 * Mantiene account, settings y templates.
 */
function reconstruirDesdeServidor(PDO $pdo, int $accountId): void
{
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Base reconstruida. Eliminados: '
            . $totalEliminados
            . ' | Nuevos: '
            . $resultado['insertados']
            . ' | Actualizados: '
