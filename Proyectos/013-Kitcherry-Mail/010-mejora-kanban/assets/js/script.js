...
// Mejora final: Kanban con 5 estados, restauración desde archivados y borradores editables.
// Ajuste: limpieza visual de espacios HTML en resúmenes.
// ==========================================================
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function limpiarResumenVista(texto) {
        const caja = document.createElement("textarea");

        caja.innerHTML = texto.toString();
        ...
        if (busquedaActual.trim() !== "") {
            ...
            resultado = resultado.filter(function (correo) {
                const textoCorreo = [
                    ...
                    correo.destinatario,
                    correo.asunto,
                    limpiarResumenVista(correo.resumen),
                    correo.cuerpo,
                    correo.tipo,
                    correo.estado,
                    correo.prioridad
    ...
    function esBorrador(correo) {
        return String(correo.carpeta).trim() === "drafts";
    }

    function abrirBorradorDesdeCorreo(correo) {
        abrirModalRedaccion("borrador", correo);
        mostrarToast("Borrador abierto para editar");
    }
    ...
    function renderizarLista() {
        ...
        lista.forEach(function (correo) {
            ...
            boton.innerHTML = `
                ...
                <p class="mail-preview">${escaparHtml(limpiarResumenVista(correo.resumen))}</p>
            ...
            boton.addEventListener("click", function () {
                if (esBorrador(correo)) {
                    abrirBorradorDesdeCorreo(correo);
                    return;
                }
    ...
    function renderizarKanban() {
        const columnas = {
            ...
            importante: document.getElementById("kanbanImportante"),
        ...
        const correosKanban = correos.filter(function (correo) {
        ...
        correosKanban.forEach(function (correo) {
            ...
            tarjeta.addEventListener("click", function () {
                ...
                if (vistaActual === "kanban") {
                    mostrarToast("Correo seleccionado en Kanban");
                }
            });
        ...
        Object.values(columnas).forEach(function (columna) {
            if (columna.children.length === 0) {
                ...
                vacio.textContent = "Sin correos en esta columna";
    ...
    function prepararKanbanDragDrop() {
        ...
        zonas.forEach(function (zona) {
            ...
            zona.addEventListener("drop", async function (evento) {
                ...
                if (!correoId || !nuevoEstado) {
                    return;
                }

                const correoMovido = correos.find(function (correo) {
                    return correo.id === correoId;
                });

                const carpetaOrigen = correoMovido
                    ? String(correoMovido.carpeta).trim()
                    : "";
                ...
                if (nuevoEstado === "archivado") {
                    ...
                    if (carpetaOrigen === "archived") {
                        await apiPost("restore_inbox", {
                            id: correoId
                        });
                    }
    ...
    function actualizarModoVistaGlobal() {
        if (vistaActual === "kanban") {
    ...
    function prepararEstadoModal(titulo, textoBotonBorrador, textoBotonEnviar) {
        composeTitle.textContent = titulo;

        if (btnSaveDraft) {
            btnSaveDraft.textContent = textoBotonBorrador;
        }

        if (composeSubmitButton) {
            composeSubmitButton.textContent = textoBotonEnviar;
        }
    }
    ...
    function abrirModalRedaccion(modo, correo = null) {
        ...
        if (modo === "nuevo") {
            prepararEstadoModal("Nuevo correo", "Guardar borrador", "Enviar correo");
        ...
        if (modo === "borrador" && correo) {
            prepararEstadoModal("Editar borrador", "Actualizar borrador", "Enviar correo");

            composeDraftId = correo.id;
            composeSourceId = null;

            composeTo.value = correo.destinatario || "";
            composeSubject.value = correo.asunto || "";
            composeEditor.innerHTML = correo.cuerpo || "<p>Escribe aquí tu mensaje...</p>";
        }

        if (modo === "responder" && correo) {
            prepararEstadoModal("Responder correo", "Guardar borrador", "Enviar correo");
        ...
        if (modo === "reenviar" && correo) {
            prepararEstadoModal("Reenviar correo", "Guardar borrador", "Enviar correo");
    ...
    btnSaveDraft.addEventListener("click", async function () {
        ...
        if (composeMode === "borrador" && composeDraftId) {
            respuesta = await apiPost("update_draft", {
                id: composeDraftId,
                to: composeTo.value,
            ...
            respuesta = await apiPost("save_draft", {
                to: composeTo.value,
                subject: composeSubject.value,
                body_html: composeEditor.innerHTML
            });
        ...
        if (respuesta) {
            carpetaActual = "drafts";
            cambiarCarpetaVisual("drafts");
            ...
            mostrarToast(composeDraftId ? "Borrador actualizado" : "Borrador guardado");
        }
    });
    ...
    composeForm.addEventListener("submit", async function (evento) {
        ...
        const draftIdAntesDeEnviar = composeDraftId;
        ...
        if (modoAntesDeEnviar === "borrador" && draftIdAntesDeEnviar) {
            carpetaActual = "sent";
            correoSeleccionadoId = null;
            ...
            mostrarToast("Borrador enviado correctamente");
            return;
        }
    ...
    btnListView.addEventListener("click", function () {
        ...
        const correoSeleccionado = obtenerCorreoSeleccionado();

        if (correoSeleccionado && !esBorrador(correoSeleccionado)) {
            const carpetaCorreo = String(correoSeleccionado.carpeta).trim();

            if (nombresCarpetas[carpetaCorreo]) {
                cambiarCarpetaVisual(carpetaCorreo);
            }
    ...
    function actualizarInterfaz() {
        ...
        if (correo && vistaActual !== "kanban" && !esBorrador(correo)) {
            emptyState.classList.add("hidden");
            detailContent.classList.remove("hidden");
            renderizarDetalle(correo);
        }

        if (correo && esBorrador(correo)) {
            ocultarDetalle();
        }
