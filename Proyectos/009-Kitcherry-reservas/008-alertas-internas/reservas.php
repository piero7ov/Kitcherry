...
function cargarReservaParaConfirmacion($pdo, $reservaId, $negocioId) {
    $stmt = $pdo->prepare("
        SELECT
            r.id,
            r.negocio_id,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $negocio) {
    ...
    // ------------------------------------------------------
    if (($_POST['accion'] ?? '') === 'enviar_confirmacion') {
        ...
        if ($reservaId <= 0) {
            ...
            try {
                $reservaConfirmacion = cargarReservaParaConfirmacion(
                    $pdo,
                    $reservaId,
                    $negocio['id']
                );
                ...
                if (!$reservaConfirmacion) {
                ...
                } elseif (empty($reservaConfirmacion['cliente_email'])) {
                    $error = 'No se puede enviar confirmación porque el cliente no tiene email.';
                } else {
                    $asunto = generarAsuntoConfirmacion($reservaConfirmacion);
                    $cuerpo = generarCuerpoConfirmacion($reservaConfirmacion);

                    enviarCorreoSMTP(
                        $reservaConfirmacion['cliente_email'],
                        $asunto,
                        $cuerpo,
                        $reservaConfirmacion['negocio_nombre'] ?: 'Kitcherry Reservas'
                    );
                    ...
                    $stmt->execute([
                        ':reserva_id' => $reservaConfirmacion['id'],
                        ':cliente_id' => $reservaConfirmacion['cliente_id'],
                        ':asunto' => $asunto,
                        ':cuerpo' => $cuerpo,
                        ':tono' => $reservaConfirmacion['tono_comunicacion'] ?: 'cercano'
                    ]);
                    ...
                    $stmt->execute([
                        ':id' => $reservaConfirmacion['id'],
                        ':negocio_id' => $negocio['id']
                    ]);

                    registrarHistorialReserva(
                        $pdo,
                        $reservaConfirmacion['id'],
                        $usuario['id'],
                        'confirmacion_enviada',
                        'Confirmación enviada por email al cliente.'
                    );

                    generarAlertasReserva($pdo, (int)$reservaConfirmacion['id']);

                    $pdo->commit();

                    $mensaje = 'Confirmación enviada correctamente.';
                    $filtroFecha = $reservaConfirmacion['fecha'];
                }
                ...
                $error = 'No se pudo enviar la confirmación. Revisa la configuración SMTP o el email del cliente.';
            }
        }
    }
