...
Entrenamiento LoRA usando CPU.

Aviso importante:
- Esta versión funciona sin GPU, pero con Qwen2.5-1.5B puede ser lenta.
- La versión recomendada es 002-entrenamiento_gpu.py.
- Se mantiene al mismo nivel lógico que la versión GPU.
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "lora-cpu")
...
def crear_prompt_y_texto_completo(tokenizer: AutoTokenizer, question: str, answer: str) -> Tuple[str, str]:
    """
    Crea el prompt en formato chat y añade la respuesta esperada.
    """
...
def main() -> None:
    ...
    print("=" * 70)
    print("KITCHERRY KITCHEN ASSISTANT - ENTRENAMIENTO CPU")
    print("=" * 70)
    ...
    if torch.cuda.is_available():
        print("\nAviso: se ha detectado GPU CUDA, pero este script usará CPU.")
        print("Para entrenar con GPU usa: python 002-entrenamiento_gpu.py")

    print("\nDispositivo utilizado: CPU")
    print("Precisión usada: torch.float32")
    ...
    # ------------------------------------------------------
    # 4. Cargar modelo base en CPU
    # ------------------------------------------------------

    print("\nCargando modelo base en CPU...")
    ...
    print("\nIniciando entrenamiento LoRA en CPU...")
    print("Aviso: puede tardar bastante. Para este modelo se recomienda entrenar con GPU.")
