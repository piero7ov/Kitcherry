...
// Archivo: 008-reconocimiento-de-voz.php
// Chat simple basado en Ollama con reconocimiento de voz
// ==========================================================
...
function limpiarRespuestaIA($texto) {
    ...
    // Eliminar etiquetas peligrosas si el modelo las genera
    $texto = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', '', $texto);
    $texto = preg_replace('/<style\b[^>]*>(.*?)<\/style>/is', '', $texto);

    // Convertir títulos Markdown tipo # Título, ## Título, etc.
    $texto = preg_replace('/^#{1,6}\s*(.*?)$/m', '<p><strong>$1</strong></p>', $texto);

    // Convertir negritas Markdown **texto** a <strong>texto</strong>
    $texto = preg_replace('/\*\*(.*?)\*\*/s', '<strong>$1</strong>', $texto);

    // Convertir viñetas Markdown "* texto" o "- texto" en saltos visuales
    $texto = preg_replace('/(^|\n)\s*[\*\-]\s+/', '<br>• ', $texto);
    ...
    // Si no trae HTML, envolverlo en párrafo
