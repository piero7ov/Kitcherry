...
// Plano editable con importación de reservas JSON y asignación manual por arrastre
// ==========================================================
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function actualizarReservasMesaSeleccionada(mesa) {
        ...
        reservasMesaSeleccionada.className = "mesa-reservas-box reservas-lista";
        reservasMesaSeleccionada.innerHTML = mesa.reservas.map(function (reserva) {
    ...
    function importarReservas(datosOriginales) {
        ...
        reservasImportadas = datosAdaptados.reservas;

        vincularReservasConMesas(true);
    ...
    function vincularReservasConMesas(actualizarEstadoMesas) {
        ...
        reservasImportadas.forEach(function (reserva) {
            ...
            if (!mesa && reserva.mesa_id) {
                mesa = obtenerMesaPorId(reserva.mesa_id);
            }
            ...
            if (mesa) {
                ...
                reserva.colocacion = {
                    ...
                    mesa_canvas_id: null,
                    pendiente: true,
                    motivo_pendiente: reserva.mesa_id === null
                        ? "Reserva sin mesa asignada"
                        : "No existe una mesa con ID " + reserva.mesa_id + " en el plano",
                    asignacion_manual: false
                };
    ...
    function actualizarPanelReservas() {
        ...
        if (!servicioImportado) {
            servicioInfo.className = "empty-state";
        ...
        servicioInfo.className = "servicio-info-card";
        ...
        if (reservasAsignadas.length === 0) {
            reservasLista.innerHTML = `
                <div class="empty-state">
                    No hay reservas vinculadas a mesas del plano.
                </div>
            `;
        }
